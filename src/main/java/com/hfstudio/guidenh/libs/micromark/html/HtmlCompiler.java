...
public class HtmlCompiler {
    ...
    public String compile(List<Tokenizer.Event> events) {
        ...
        while (++index < events.size()) {
            ...
            if (lineEndingStyle == null && (token.type.equals("lineEnding") || token.type.equals("lineEndingBlank"))) {
                ...
                lineEndingStyle = event.context()
                    .sliceSerialize(token);
            }
    ...
    private void lineEnding() {
        raw((lineEndingStyle != null ? (lineEndingStyle) : ("\n")));
    }
    ...
    private void onexitcodefencedfence() {
        int count = (data.fencesCount != null ? (data.fencesCount) : (0));
        if (count == 0) {
            tag(">");
    ...
    private void onexitflowcode() {
        ...
        if (count != null && count < 2 && !tightStack.isEmpty() && !data.lastWasTag) {
            lineEnding();
        }
    ...
    private void onexitmedia() {
        ...
        var id = (media.referenceId != null ? (media.referenceId) : (media.labelId));
        ...
        if (media.image) {
            tag(
                "<img src=\""
                    + SanitizeUri
                        .sanitizeUri(context.destination, options.isAllowDangerousProtocol() ? null : protocolSrc)
                    + "\" alt=\"");
            raw(media.label);
            tag("\"");
        } else {
            tag(
                "<a href=\""
                    + SanitizeUri
                        .sanitizeUri(context.destination, options.isAllowDangerousProtocol() ? null : protocolHref)
                    + "\"");
        }
    ...
    private void onexitatxheadingsequence(HtmlContext context, Token token) {
        ...
        data.headingRank = context.sliceSerialize(token)
            .length();
        lineEndingIfNeeded();
    ...
    private void onexitcharacterreferencevalue(HtmlContext context, Token token) {
        ...
        value = data.characterReferenceType != null
            ? NumericCharacterReference.decodeNumericCharacterReference(
                value,
    ...
    private void onexitautolinkprotocol(HtmlContext context, Token token) {
        ...
        tag(
            "<a href=\"" + SanitizeUri.sanitizeUri(uri, options.isAllowDangerousProtocol() ? null : protocolHref)
                + "\">");
        raw(encode(uri));
        tag("</a>");
    }
