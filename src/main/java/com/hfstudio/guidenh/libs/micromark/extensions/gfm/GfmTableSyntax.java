package com.hfstudio.guidenh.libs.micromark.extensions.gfm;
    ...
    private List<Tokenizer.Event> resolveTable(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        while (++index < events.size()) {
            ...
            if (inRow) {
                ...
                (token.type.equals("tableCellDivider") || token.type.equals("tableRow")) && contentEnd != -1) {
                    ...
                    content.start = events.get(contentStart)
                        .token().start;
                    content.end = events.get(contentEnd)
                        .token().end;
            ...
            if (events.get(index)
                ...
                && (token.type.equals("tableCellDivider")
                    || (token.type.equals("tableRow") && (cellStart + 3 < index || !events.get(cellStart)
                        .token().type.equals(Types.whitespace))))) {

                var cell = new Token();
                cell.type = inDelimiterRow ? "tableDelimiter" : inHead ? "tableHeader" : "tableData";
                cell.start = events.get(cellStart)
                    .token().start;
                cell.end = events.get(index)
                    .token().end;
            ...
            if (token.type.equals("tableDelimiterRow")) {
                inDelimiterRow = events.get(index)
                    .isEnter();
    ...
    private State tokenizeTable(TokenizeContext context, Tokenizer.Effects effects, State ok, State nok) {
        class StateMachine {
            ...
            State inCellContentHead(int code) {
                // EOF, whitespace, pipe
                if (code == Codes.eof || code == Codes.verticalBar || CharUtil.markdownLineEndingOrSpace(code)) {
                    effects.exit("temporaryTableCellContent");
                    return cellBreakHead(code);
                }

                effects.consume(code);
                return code == Codes.backslash ? this::inCellContentEscapeHead : this::inCellContentHead;
            }
            ...
            State inCellContentBody(int code) {
                ...
                effects.consume(code);
                return code == Codes.backslash ? this::inCellContentEscapeBody : this::inCellContentBody;
            }
            ...
            State atRowEndBody(int code) {
                ...
                return effects.check
                    .hook(
                        ...
                        effects.attempt.hook(
                            construct,
                            FactorySpace.create(effects, this::rowStartBody, Types.linePrefix, Constants.tabSize),
                            this::tableBodyClose))
                    .step(code);
            }
            ...
            State tokenizeRowEnd(TokenizeContext context, Tokenizer.Effects effects, State ok, State nok) {
                class RowEndStateMachine {
                    ...
                    State prefixed(int code) {
                        ...
                        if (context.isOnLazyLine() || code == Codes.eof || CharUtil.markdownLineEnding(code)) {
                            return nok.step(code);
                        }
                        ...
                        return effects.check.hook(context.getParser().constructs.flow, c -> {
