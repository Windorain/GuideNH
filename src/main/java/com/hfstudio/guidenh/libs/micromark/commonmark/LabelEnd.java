...
public class LabelEnd {
    ...
    public static List<Tokenizer.Event> resolveAll(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        while (++index < events.size()) {
            var token = events.get(index)
                .token();

            if (token.type.equals(Types.labelImage) || token.type.equals(Types.labelLink)
    ...
    public static List<Tokenizer.Event> resolveToLabelEnd(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        while (index-- > 0) {
            ...
            if (open != null) {
                ...
                // links.
                if (events.get(index)
                    .isEnter() && token.type.equals(Types.labelLink)) {
                    token._inactive = true;
                }
            } else if (close != null) {
                if (events.get(index)
                    .isEnter() && (token.type.equals(Types.labelImage) || token.type.equals(Types.labelLink))
                    && !token._balanced) {
                    open = index;
        ...
        label.start = events.get(open)
            .token().start;
        label.end = events.get(close)
            .token().end;

        var text = new Token();
        text.type = Types.labelText;
        text.start = events.get(open + offset + 2)
            .token().end;
        text.end = events.get(close - 2)
            .token().start;
    ...
    public static class StateMachine {
        ...
        public StateMachine(TokenizeContext context, Tokenizer.Effects effects, State ok, State nok) {
            ...
            this.ok = ok;
            this.nok = nok;
            this.index = context.getEvents()
                .size();

            // Find an opening.
            while (index-- > 0) {
                if ((context.getEvents()
                    .get(index)
        ...
        private State start(int code) {
            ...
            defined = context.getParser().defined.contains(
                NormalizeIdentifier.normalizeIdentifier(context.sliceSerialize(labelStart.end, context.now())));
        ...
        private State afterLabelEnd(int code) {
            ...
            if (code == Codes.leftParenthesis) {
                return effects.attempt.hook(resourceConstruct, ok, defined ? ok : this::balanced)
                    .step(code);
            }
            ...
            if (code == Codes.leftSquareBracket) {
                return effects.attempt
                    .hook(
                        ...
                        ok,
                        defined ? effects.attempt.hook(collapsedReferenceConstruct, ok, this::balanced)
                            : this::balanced)
                    .step(code);
            }
    ...
    public static class ResourceStateMachine {
        ...
        private State destinationAfter(int code) {
            return CharUtil.markdownLineEndingOrSpace(code) ? FactoryWhitespace.create(effects, this::between)
                .step(code) : end(code);
        }
    ...
    public static class FullReferenceStateMachine {
        ...
        private State afterLabel(int code) {
            ...
            return context.getParser().defined.contains(
                NormalizeIdentifier.normalizeIdentifier(lastTokenText.substring(1, lastTokenText.length() - 1)))
                    ? ok.step(code)
                    : nok.step(code);
        }
    }
