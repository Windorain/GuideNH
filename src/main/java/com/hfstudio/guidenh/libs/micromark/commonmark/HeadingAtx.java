...
public class HeadingAtx {
    ...
    public static List<Tokenizer.Event> resolveHeadingAtx(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        if (events.get(contentStart)
            .token().type.equals(Types.whitespace)) {
            contentStart += 2;
        }

        // Suffix whitespace, part of the closing.
        if (contentEnd - 2 > contentStart && events.get(contentEnd)
            .token().type.equals(Types.whitespace)) {
            contentEnd -= 2;
        }

        if (events.get(contentEnd)
            .token().type.equals(Types.atxHeadingSequence)
            && (contentStart == contentEnd - 1 || (contentEnd - 4 > contentStart && events.get(contentEnd - 2)
                .token().type.equals(Types.whitespace)))) {
        ...
        if (contentEnd > contentStart) {
            ...
            text.start = events.get(contentStart)
                .token().start;
            text.end = events.get(contentEnd)
                .token().end;
    ...
    public static class StateMachine {
        ...
        private State headingBreak(int code) {
            ...
            if (CharUtil.markdownSpace(code)) {
                return FactorySpace.create(effects, this::headingBreak, Types.whitespace)
                    .step(code);
            }
        ...
        private State data(int code) {
            if (code == Codes.eof || code == Codes.numberSign || CharUtil.markdownLineEndingOrSpace(code)) {
