...
public class Attention {
    ...
    public static List<Tokenizer.Event> resolveAllAttention(List<Tokenizer.Event> events, TokenizeContext context) {
        ...
        while (++index < events.size()) {
            ...
            if (event.type() == Tokenizer.EventType.ENTER && event.token().type.equals("attentionSequence")
                && event.token()._close) {
                open = index;
                ...
                while (open-- > 0) {
                    ...
                    if (openEvent.type() == Tokenizer.EventType.EXIT
                        ...
                        context.sliceSerialize(openEvent.token())
                            .charAt(0)
                            == context.sliceSerialize(event.token())
                                .charAt(0)) {
                        ...
                        if ((openEvent.token()._close || event.token()._open)
                            ...
                            && ((openEvent.token().end.offset() - openEvent.token().start.offset()
                                + event.token().end.offset()
                                - event.token().start.offset()) % 3) == 0) {
                            continue;
                        }

                        // Number of markers to use from the sequence.
                        use = openEvent.token().end.offset() - openEvent.token().start.offset() > 1
                            && event.token().end.offset() - event.token().start.offset() > 1 ? 2 : 1;
    ...
    static class StateMachine {
        ...
        State sequence(int code) {
            ...
            var close = before == 0 || (before == Constants.characterGroupPunctuation && after != 0)
