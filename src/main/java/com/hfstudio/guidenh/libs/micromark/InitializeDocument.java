...
public class InitializeDocument {
    ...
    public static class StateMachine {
        ...
        public State start(int code) {
            ...
            if (continued < stack.size()) {
                ...
                return effects.attempt
                    .hook(item.construct().continuation, this::documentContinue, this::checkNewContainers)
                    .step(code);
            }
        ...
        private State documentContinue(int code) {
            ...
            if (Boolean.TRUE.equals(
                ...
                var indexBeforeExits = context.getEvents()
                    .size();
                ...
                context.getEvents()
                    .addAll(indexBeforeFlow + 1, eventsToMove);
        ...
        private State checkNewContainers(int code) {
            ...
            return effects.check.hook(containerConstruct, this::thereIsANewContainer, this::thereIsNoNewContainer)
                .step(code);
        }
        ...
        private State thereIsNoNewContainer(int code) {
            context.getParser().lazy.put(
                context.now()
                    .line(),
                continued != stack.size());
            lineStartOffset = context.now()
                .offset();
            return flowStart(code);
        }
        ...
        private State documentContinued(int code) {
            // Try new containers.
            context.setContainerState(new Tokenizer.ContainerState());
            return effects.attempt.hook(containerConstruct, this::containerContinue, this::flowStart)
                .step(code);
        }
        ...
        private void writeToChild(Token token, boolean eof) {
            ...
            token.previous = childToken;
            if (childToken != null) childToken.next = token;
            childToken = token;
            ...
            if (context.getParser()
                .isLazyLine(token.start.line())) {
                var index = childFlow.getEvents()
                    .size();

                while (index-- > 0) {
                    var childFlowToken = childFlow.getEvents()
                        .get(index)
                        .token();
                    if (
    ...
    public static State tokenizeContainer(TokenizeContext context, Tokenizer.Effects effects, State ok, State nok) {
        return FactorySpace.create(
            ...
            Types.linePrefix,
            context.getParser().constructs.nullDisable.contains(Types.codeIndented) ? Integer.MAX_VALUE
                : Constants.tabSize);
    }
}
