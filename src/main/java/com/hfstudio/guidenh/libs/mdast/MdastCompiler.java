...
import com.hfstudio.guidenh.libs.mdast.model.MdAstReferenceType;
...
import com.hfstudio.guidenh.libs.micromark.DecodeString;
...
public class MdastCompiler implements MdastContext {
    ...
    MdastCompiler(MdastOptions options) {
        var extensionBuilder = MdastExtension.builder()
            .canContainEol("emphasis", "fragment", "heading", "paragraph", "strong")
    ...
    MdAstRoot compile(List<Tokenizer.Event> events) {
        ...
        tree.position = new MdAstPosition().withStart(
            ...
            .withEnd(
                point(
                    !events.isEmpty() ? events.get(events.size() - 2)
                        .token().end : makePoint(1, 1, 0)));
        ...
        while (++index <= length) {
            ...
            if ((containerBalance == 0 && event.isEnter() && tokenType.equals(Types.listItemPrefix))
                || (containerBalance == -1 && event.isExit()
                    && (tokenType.equals(Types.listUnordered) || tokenType.equals(Types.listOrdered)))) {
                if (listItem != null) {
                    var tailIndex = index;
                    lineIndex = null;
                    ...
                    // Fix position.
                    listItem.end = (lineIndex != null && lineIndex != 0) ? events.get(lineIndex)
                        .token().start : event.token().end;
    ...
    public MdAstNode exit(Token token, OnExitError onExitError) {
        ...
        if (open == null) {
            ...
            if (onExitError != null) {
                ...
                MdastContext.OnEnterError handler = open.onError() != null ? open.onError() : this::defaultOnError;
    ...
    private void onenterlistitemvalue(MdastContext context, Token token) {
        if (expectingFirstListItemValue) {
            ...
            ancestor.start = Integer.parseInt(this.sliceSerialize(token), Constants.numericBaseDecimal);
    ...
    private void onexitlineending(MdastContext ignored, Token token) {
        ...
        if (!setextHeadingSlurpLineEnding && extension.canContainEols.contains(context.type())) {
    ...
    private void onexitlink() {
        ...
        if (inReference) {
            ...
            ref.referenceType = (referenceType != null ? (referenceType) : (MdAstReferenceType.SHORTCUT));
    ...
    private void onexitimage() {
        ...
        if (inReference) {
            ...
            imgRef.referenceType = (referenceType != null ? (referenceType) : (MdAstReferenceType.SHORTCUT));
    ...
    private void onexitlabeltext(MdastContext context, Token token) {
        ...
        for (int i = stack.size() - 2; i >= 0; i--) {
            ...
            if (ancestor instanceof LinkOrLinkReference link) {
                ...
                link.identifier = NormalizeIdentifier.normalizeIdentifier(string)
                    .toLowerCase();
                return;
            } else if (ancestor instanceof ImageOrImageReference image) {
                image.label = DecodeString.decodeString(string);
                image.identifier = NormalizeIdentifier.normalizeIdentifier(string)
                    .toLowerCase();
                return;
            }
        }
    ...
    private void onexitreferencestring(MdastContext context, Token token) {
        ...
        if (node instanceof LinkOrLinkReference ref) {
            ref.label = label;
            ref.identifier = NormalizeIdentifier.normalizeIdentifier(this.sliceSerialize(token))
                .toLowerCase();
    ...
    private void onexitcharacterreferencevalue(MdastContext context, Token token) {
        ...
        if (type != null) {
            value = NumericCharacterReference.decodeNumericCharacterReference(
                data,
                type == CharacterReferenceType.characterReferenceMarkerNumeric ? Constants.numericBaseDecimal
